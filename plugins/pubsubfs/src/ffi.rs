...
use fs9_sdk::{Capabilities, FileType, FsError, OpenFlags};
use fs9_sdk_ffi::{
...
use crate::{PubSubFsConfig, PubSubFsProvider};

fn systemtime_to_timestamp(time: std::time::SystemTime) -> i64 {
...
#[cfg(test)]
pub(crate) unsafe fn create_provider_for_test(
    config: *const c_char,
    config_len: size_t,
) -> *mut c_void {
    create_provider(config, config_len)
}

#[cfg(test)]
pub(crate) unsafe fn destroy_provider_for_test(provider: *mut c_void) {
